        ...
        <table class="add_tab">
            ...
            <tr>
                ...
                <td>
                    <select name="cate_id">
                        @foreach ($data as $k)
                            <option value="{{$k->cate_id}}" @if ($field->cate_id == $k->cate_id) selected @endif>{{$k->_cate_name}}</option>
                        @endforeach
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <th>缩略图：</th>
                <td>
                    <input type="text" size="50" name="art_thumb" src="{{$field->art_thumb}}">
                    <input id="file_upload" name="file_upload" type="file" multiple="true">
                    <script src="{{asset('resources/org/uploadify/jquery.uploadify.min.js')}}" type="text/javascript"></script>
                    <link rel="stylesheet" type="text/css" href="{{asset('resources/org/uploadify/uploadify.css')}}">
                    <script type="text/javascript">
                        <?php $timestamp = time();?>
                                {{--alert("{{url('admin/upload')}}");--}}
                        $(function() {
            ...
            <tr>
                ...
                <td>
                    ...
                    <script id="editor" name="art_content" type="text/plain" style="width:860px;height:100px;">{!!$field->art_content!!}</script>
